...
<div bgcolor='#ffffff'>
    ...
    <%
        DAO dao = new DAO();
        Aviary aviary;
        ArrayList<Aviary> list;
        if (request.getParameter("action") != null) {
            list = dao.getAllAviary();
            int id = Integer.parseInt(request.getParameter("action"));
            aviary = (Aviary) dao.getAviaryById(id);
        } else {
            aviary = null;
        }
    %>
    <form id="t" method='POST' action='aviary.jsp'>
        <input type="hidden" readonly name="idAviary"
               value="<%if(aviary!=null)out.print(aviary.getIdAviary());%>"/>
        Название: <input required value="<%if(aviary!=null) out.print(aviary.getTitle()); %>" name="title"/>
        <br><br>
        Размер: <input required value="<%if(aviary!=null) out.print(aviary.getSize()); %>" name="size"/>
        <br><br>
